...
- `StateMachine` — states are roots, transitions are relations;
  generated by `#[derive(amenable_derive::StateMachine)]` from explicit
  `#[state_machine(..)]` declarations, not hand-implemented (see
  `docs/STATE_MACHINE_DERIVATION_PLAN.md`). Replaces an earlier
  `StateMachine`/`Amenable` trait pair entirely — no `Amenable` trait
  exists anymore.
