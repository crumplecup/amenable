...
#[test]
fn drain_model_preserves_empty_utf8_content() {
    let source = KaniUtf8Buffer::<2>::new([0, 0], 0).expect("empty bytes are valid utf8");
...
#[test]
fn drain_model_preserves_bounded_utf8_content_in_order() {
    let source = KaniUtf8Buffer::<2>::new(*b"ab", 2).expect("ascii is valid utf8");
    let observation = KaniStringDrainObservation::new(source);
