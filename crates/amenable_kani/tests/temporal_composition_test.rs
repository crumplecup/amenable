...
//! real `ClassifiedWitness<KaniVerifier>` — its machine-checked leaves
//! (`amenable_kani::time`) and its citation-only structural leaves
//! (`amenable_time::structural_witness`) compose into one product proof
//! whose support summary honestly reports the checked / trusted / trivial
