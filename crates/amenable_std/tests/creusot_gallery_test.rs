...
#[test]
fn all_nine_translator_findings_are_registered_and_distinct() {
    ...
    assert_eq!(
        cases.len(),
        9,
        "expected exactly the 9 findings from this session's real pipeline work: {cases:#?}"
    );
    ...
    assert_eq!(
        ...
        "seven findings were real, diagnosed translation errors"
    );
