...
/// The minimal self-referential root for this file's `Exchange` finding.
...
/// The non-root evidence type this file's `Exchange` edge establishes,
/// via [`ExchangeRootToken`].
...
/// The proof token witnessing [`ExchangeRoot`].
#[derive(Clone, Copy)]
...
/// The proof token witnessing [`ExchangeNext`].
#[derive(Clone, Copy)]
...
/// The `Sidecar<V>`-carrier this file's finding exercises.
...
impl<T, Token> GalleryExchanged<T, Token> {
    /// Builds a carrier from a payload and its proof token.
...
/// The type `verus_exchange!` below attaches the real `Exchange` edge to
