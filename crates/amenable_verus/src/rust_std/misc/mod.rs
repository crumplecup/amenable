...
/// `mod` instead of ten scattered per-carrier ones -- `mod` declarations
/// are never scanned for `#[cfg]` scatter (gating a whole module is the
/// recommended shape), so collapsing every carrier's spec-only names into
/// one gated module here, then flattening them back into this module's
/// own namespace with a single further-gated glob, keeps every name at
/// its original `misc::name` path while paying the scatter cost once.
#[cfg(verus_keep_ghost)]
mod ghost_reexports {
    pub use super::box_carrier::box_derefs_and_writes_through;
    pub use super::cow_carrier::{
        cow_into_owned_preserves_variant_value, i32_to_owned_spec_is_identity,
    };
    pub use super::env_carrier::args_model_count_matches_program_plus_extra;
    pub use super::fmt_carrier::{
        fmt_arguments_result_matches_display_token,
        fmt_debug_list_result_matches_entries_in_brackets,
        fmt_debug_map_result_matches_key_value_pair,
        fmt_debug_set_result_matches_entries_in_braces,
        fmt_debug_struct_result_matches_named_fields,
        fmt_debug_tuple_result_matches_positional_fields, fmt_from_fn_result_matches_display_token,
    };
    pub use super::layout_carrier::{
        ExLayout, ExLayoutError, from_size_align_rejects_a_non_power_of_two_alignment,
        is_power_of_two_spec,
    };
    pub use super::option_carrier::{option_is_none, option_wraps_the_given_value};
    pub use super::result_carrier::{
        result_wraps_the_given_err_value, result_wraps_the_given_ok_value,
    };
    pub use super::std_time_carrier::{
        duration_new_result_matches, duration_new_secs_headroom_holds,
    };
    pub use super::type_id_carrier::{
        ExTypeId, type_id_eq_matches_identity, type_id_of_matches_spec,
    };
}
#[cfg(verus_keep_ghost)]
