...
`TemporalFormatter<V>`, `TemporalZoneFactory<V>`,
`TemporalConversionFactory<V>`, `TemporalIntervalFactory<V>`). The
`Exchange` impls live in the backend crate (orphan rule; generated by
`#[capture_exchange_body]`). Step 2a: 23 top-level descriptors + their
transitive enums gain `Default` / `Evidence`. Step 2b: all 24 `ParsedX`
sidecars + 12 per-method composite propositions + 24 `#[establish]`
tokens + the `TemporalParser<V>` bundle. Step 3: 36 `Formatted*` output
sidecars + 36 emission-proof composites + 3 formatter-only `Parsed*`
inputs + 36+3 tokens + the `TemporalFormatter<V>` bundle;
`TemporalInputReceived` lost its unconditional `Witness` impl (now
backend-provided). Step 4: the 3 factory seams — 9 transition methods
...
- [x] **Step 2a (2026-09-09):** the 22 remaining top-level descriptors
      that become a `Sidecar::Primary` gain `#[derive(Default,
      amenable_derive::Evidence)] #[evidence(basis = "Self")]`; their
      transitive field types gain `Default` (fieldless enums via
      `#[default]`, 22 data-carrying enums via a generated hand
      `impl Default` on the first variant). No construction-time
      validation added — a degenerate `Default` is rejected by a proof.
- [x] **Step 2b (2026-09-09):** all 24 `TemporalParser<V>` supertrait
      bounds. `src/exchange/parse.rs` (24 `ParsedX` `#[derive(Sidecar)]`
      structs), `parse_props.rs` (12 per-method composite propositions —
      the multi-proof methods' 2–9 `Established`/`*ProofBranch` sidecars
      folded into one `#[derive(Evidence, Witness)]` struct),
      `establish.rs` (24 `#[amenable_derive::establish]` tokens),
      `traits/parse.rs` (`TemporalParser<V>` = 24 `Exchange<RawInput,
      ParsedX, V>` supertrait bounds + 24 `where <Prop>: Witness<V>`
      bounds + the blanket). Generated from `elicit_temporal`. 6 tests.
- [ ] **Step 2c:** a codegen macro that generates a backend's 24
      `Exchange` impls from its raw parse fns (`#[capture_exchange_body]`
      per method is the manual form).
- [x] **Step 3 (2026-09-09):** `TemporalFormatter<V>` (36 methods). Input
      is a `Parsed*` sidecar (parser out = formatter in), reusing the 24
      parser outputs + 3 formatter-only additions (`ParsedDuration`,
      `ParsedRecurringInterval`, `ParsedIxdtfZonedTimestamp`). Output is a
      `Formatted<Method>` `#[derive(Sidecar)]` struct — `#[sidecar(primary)]
      FormattedTemporalText` (String newtype, the emitted wire text) over
      a `<Method>Formatted` emission-proof composite (the method's output
      proofs folded the Phase-3 way). `src/exchange/format.rs` (36
      sidecars), `format_props.rs` (36 composites), `establish.rs`
      extended (+36 output tokens, credentialled by the input's token; +3
      input tokens), `traits/format.rs` (36 `Exchange<ParsedX,
      FormattedY, V>` supertrait bounds + 27 input + 36 output `where
      <Prop>: Witness<V>` bounds + the blanket). `markers.rs`: dropped the
      unconditional `impl Witness for TemporalInputReceived` (its
      `Witness<V>` is backend-provided like a contract's), added
      `FormattedTemporalText`. Generated from `elicit_temporal`. 7 tests.
- [x] **Step 4 (2026-09-09):** `TemporalZoneFactory<V>` (5),
      `TemporalConversionFactory<V>` (4), `TemporalIntervalFactory<V>`
      (4). 9 transition methods each fold to a `*Request` primary +
      `*Preconditions` proposition (the caller's `Established<_>` sidecars,
      folded) in, and a `*Established` proposition (return-tuple proofs,
      folded the Phase-3 way; `*Evidence` bundles already merged into
      their `*Valid` are dropped) out — `*Input` / `*Output`
      `#[derive(Sidecar)]` structs in `src/exchange/{zone,conversion,
      interval}.rs`, two `#[establish]` edges each in
      `factory_establish.rs` (`*PreconditionsToken` ← `TemporalInputToken`,
      `*EstablishedToken` ← `*PreconditionsToken`). The 4 parser-style
      factory methods (`resolve_named_zone` + the 3 interval parses) are
