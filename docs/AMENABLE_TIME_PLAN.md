...
Phase 4 Steps 1–3 (2026-09-09) — the exchange surface, in
`src/exchange/`. Every `elicit_temporal` return tuple *is* a sidecar;
`amenable_time` ships the sidecar types + `Establish` edges +
`TemporalParser<V>` / `TemporalFormatter<V>` (traits whose supertrait
bundle *is* the 24 parse / 36 format exchanges). The `Exchange` impls
live in the backend crate (orphan rule; generated by
...
backend-provided). All generated from `elicit_temporal`.
...
trait) were reverted. Next: `TemporalZoneFactory` / `TemporalConversionFactory`
/ `TemporalIntervalFactory` / `TemporalCalConnectFactory`, then Step 2c
(a codegen macro for a backend's `Exchange` impls).
