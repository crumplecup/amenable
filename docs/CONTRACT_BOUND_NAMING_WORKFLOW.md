...
**Status:** 🔲 Ongoing — this document describes the current naming
mechanism and workflow. Treat per-crate backlog counts as ephemeral and
re-run the scan before choosing work; this doc intentionally omits
...
1. Run the workflow above starting from step 1 so the checklist and
   registry dump reflect the current tree.
2. Pull from the top of the target crate's duplicate-cluster list for
   leverage, but always read the real sites before deciding whether the
   cluster is one shared claim or several heterogeneous ones.
3. Name every real bound you touch. Heterogeneity changes the mechanism
   (shared type vs. per-carrier registration), not whether the bound
   should be named.
4. Re-scan after each batch and trust the fresh checklist over any older
