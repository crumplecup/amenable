...
trifecta"): `VERUS_EXCHANGE_PROOF_DERIVATION_PLAN.md`'s own Step 6,
`exchange_support::verus_ensures!` — necessarily a `macro_rules!`
macro rather than a proc-macro crate like `amenable_derive`, since
...
`VERUS_EXCHANGE_PROOF_DERIVATION_PLAN.md`'s own Step 6,
`exchange_support::verus_ensures!` — completing what the user called
the "trifecta": Kani's contract-routing generated by a proc-macro
attribute, Verus's generated by a `macro_rules!` macro (a proc-macro
crate structurally cannot resolve under `verus --crate-type=lib` at
all), and Creusot's own consistency test (Step 2) keeping its
hand-written mirror honest against the real Kani source. Nothing
further is queued — extending this pattern to more `Exchange` edges
beyond `Stoplight`; wiring `Requires<KaniVerifier>`/`Requires<V>` for a
real (non-trivial) precondition once one exists; or generating the
surrounding `Evidence`/`Witness`/`ProofToken`/`Establish`/`Exchange`
boilerplate itself on either backend (not just the `Ensures<V>` wiring
both macros already cover) are all real future directions, but none
should be started without explicit new direction, matching this plan's
pacing throughout.
