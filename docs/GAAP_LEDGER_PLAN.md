...
themselves aren't connected to those Creusot predicates yet. Step 4
(Verus) is done, and *is* connected to `Ledger::validate`'s/`::commit`'s
real bodies: `amenable_verus::gallery::ledger_exchange` proves the same
`Validated`/`Committed` biconditionals a third way, with the real
bodies captured verbatim by the same codegen mechanism Creusot's own
`Stoplight` companions already used, extended to a second group (see
Step 4 below for the real macro-hygiene fix and two real Verus-
translator gaps this surfaced, none previously exercised). Along the way,
...
exchange.rs`'s *pre-codegen* shape -- the same mistake Step 3's own
Creusot section made and corrected. Caught immediately, before any
hand-written proof content existed: "by hand is the wrong approach, use
the derives." Built instead exactly like `stoplight_exchange.rs`'s
*current*, post-codegen shape: a new `amenable_verus::gallery::
ledger_exchange` module with real `verus_ensures!` calls (the hand-
authored predicate content, matching the real Kani biconditional and
the real Creusot `_holds` predicate exactly) plus `Ledger::validate`'s/
`::commit`'s real bodies captured verbatim and generated by `amenable::
emit-verus-exchange-companions` -- the same `amenable_core::
ExchangeEdgeRecord` registry Creusot's own companions read, extended
(previously filtered to `self_ty == "Stoplight"` only) to route each
explicitly-allowed edge into its own gallery subdirectory
(`stoplight_exchange/`, `ledger_exchange/`) rather than writing a
second group's edges as dead files into the first group's directory --
confirmed the hard way: an earlier version of the fix keyed on
`self_ty` alone and silently wrote `reject.rs`/`rollback.rs` into
`ledger_exchange/` before `include!`-scope filtering was added too.

**Real, novel findings, none previously exercised by `Stoplight`'s own
three trivial (`result.is_ok()`) edges:**

1. A real macro-hygiene gap in `verus_exchange!` itself: `validate`'s
   real body is the first captured body to reference `self.method(..)`
   at all, and the macro's own template hardcoded `fn exchange(&self,
   ..)` -- a different hygiene context from `self` as written in the
   generated caller's own text. Fixed the same way `$input_param` was
   fixed in `gallery::exchange_macro_generated`: the receiver name is
   now itself a macro argument (`$self_param:ident`).
2. Two real Verus-translator gaps in the body itself: `.map_err(
   TransferError::NegativeAmount)` (a bare tuple-variant constructor
   used as a function value) and `.map_err(|(balance, required)| ..)`
   (a destructuring closure parameter) each hit a genuine "does not yet
   support" error. Fixed at the source, in `amenable_kani::ledger`
   itself: two small real helper methods (`Ledger::negative_amount`/
   `::insufficient_funds`) replace both call sites -- a closure
   wrapping the constructor was tried first and rejected too, for a
   real, un-`#[allow]`-able `clippy::redundant_closure` failure.
   Re-verified with real `cargo kani` afterward (`0 of 489 failed`,
   both affected harnesses) to confirm the refactor changed nothing
   about what Kani proves.
3. `Result::map_err`'s own `vstd` spec states its postcondition purely
   in terms of the passed function's own declared `ensures` -- every
   accessor/constructor the captured body chains through (`Amount::
   value`, `TransferPayload::from`/`::to`/`::amount`, `Sidecar::
   primary`, `Transfer::new`, plus the two new helpers above) needed
   one added before `validate`'s own postcondition could be proven at
   any of its three real exit points. Ordinary modular verification,
   not a translator limitation -- `Stoplight`'s own trivial edges never
   needed to chain through real payload data this way.
4. `Committed`'s `debit + credit == 0` claim needed `i128` widening, not
   `int` (the `verus_ensures!` macro splices its predicate into both a
   spec fn and an exec fn, and `int`/`nat` are ghost-only) or `wrapping_
   neg`/`wrapping_add` (no `vstd` specification for either) -- `i128`
   arithmetic sidesteps both, and needs no precondition at all (`i64::
   MIN` widened to `i128` before negating never overflows), genuinely
   stronger than the real Kani/Creusot proofs, which each needed a real
   `amount > 0` precondition for the identical claim.

**Verified for real**: `verus --crate-type=lib crates/amenable_verus/
src/lib.rs` -- `458 verified, 0 errors`. A real, injected bug
(`Validated`'s `Ok` arm loosened from `amount > 0` to `amount >
1_000_000`) produced a precise `postcondition not satisfied` failure at
the exact generated `Ok(Transfer::new(..))` return, confirming the
claim is real and checked; reverted and re-verified clean. Full
workspace `check`/`fmt --check`/`clippy --all-targets --all-features -D
warnings`/`test --all-features` all clean too.

**Scope matches Step 3's own exactly**: `Pending -> Validated` and
`Validated -> Committed` only, not `reject`/`rollback` (`Rejected<
