...
mechanism; `amenable_kani` now in progress (771 → 438 sites: two real
`elicit_doc` matcher bugs fixed (a `>>` tokenization mismatch, and a
missing negated-call shape) plus fourteen clusters named -- seven
...
not `Ensures`), three non-generic (`EmptiedContainerReportsEmpty`,
`FallibleOperationReportsFailure`, `StrongCountTracksLiveReferences`),
a per-width pair (`RustStdStandard<i32>`/`RustStdStandard<u64>`'s own
