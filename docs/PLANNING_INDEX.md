...
mechanism; `amenable_kani` now in progress (771 → 446 sites: two real
`elicit_doc` matcher bugs fixed (a `>>` tokenization mismatch, and a
missing negated-call shape) plus thirteen clusters named -- seven
...
not `Ensures`), two non-generic (`EmptiedContainerReportsEmpty`,
`FallibleOperationReportsFailure`), a per-width pair
