...
**Status:** ✅ Steps 0 through 7 done and verified — the real, unmodified
...
the `ensures(...)` clauses route through registered `Ensures<
GalleryVerifier>` contract types, those contract types are generated
by `exchange_support::verus_ensures!`, and the `Witness<
GalleryVerifier>`/`Exchange<..>` impls themselves are generated by
`exchange_support::verus_exchange!` — only each edge's real transition
body stays hand-authored: `verus --crate-type=lib` reports `418
verified, 0 errors`. Closes the Verus
