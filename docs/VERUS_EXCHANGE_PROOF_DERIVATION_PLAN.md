...
contract types rather than restated inline (Step 5) and those contract
types themselves now generated by a real `macro_rules!` macro,
`exchange_support::verus_ensures!` (Step 6), rather than hand-written
per edge: `verus --crate-type=lib crates/amenable_verus/src/lib.rs`
reports `402 verified, 0 errors`. Landed in four commits: `ce77446`
(the two real trait-family fixes plus the permanent gallery
infrastructure), `5eb3566` (the full `Stoplight` cycle extension),
Step 5 (contract-bound `ensures`), and Step 6 (macro-generated
contract wiring). Full workspace `fmt`/`check`/
...
- **Extending beyond `Stoplight`** — the same generalization question
  ...
  `verus_ensures!` generalizes the `Ensures<V>`-wiring piece; nothing
  yet generates the surrounding `Evidence`/`Witness`/`ProofToken`/
  `Establish`/`Exchange` boilerplate itself, matching this whole
  lineage's own pacing discipline.
...
- **`#[path]` mod-inclusion works for bringing real, unmodified,
  ...
  needed — confirmed real and load-bearing (`402 verified, 0 errors`
...
Steps 0 through 6 are complete. One real next step remains, not
...
Generating the surrounding `Evidence`/`Witness`/`ProofToken`/
`Establish`/`Exchange` boilerplate itself (not just the `Ensures<V>`
wiring `verus_ensures!` already covers) would be a further
generalization step, also not started.
