...
DERIVATION_PLAN.md`'s own Kani-side Step 7 uses. As of Step 8, even
the `verus_exchange!(..)` call sites themselves are no longer
hand-typed: a real codegen layer (`amenable::verus_exchange_export`/
`emit-verus-exchange-companions`) reads `amenable_core::
ExchangeEdgeRecord` — the same registry Creusot's own generator reads
(`EXCHANGE_PROOF_DERIVATION_PLAN.md`'s Step 9) — and writes the three
`crate::exchange_support::verus_exchange!(..)` invocations verbatim
from Kani's own captured transition bodies. `verus
--crate-type=lib crates/amenable_verus/src/lib.rs` reports `419
verified, 0 errors` (up from `418`; `StoplightError` gained `#[derive(
Debug, Clone, Copy)]`, one more verified item). Landed in six commits:
`ce77446` (the two real trait-family fixes plus the permanent gallery
infrastructure), `5eb3566` (the full `Stoplight` cycle extension),
Step 5 (contract-bound `ensures`), Step 6 (macro-generated contract
wiring), Step 7 (macro-generated `Witness`/`Exchange` scaffolding),
and Step 8 (codegen for the call sites themselves). Full workspace
`fmt`/`check`/`clippy --all-features -D warnings`/`test` clean
throughout, `just check-all-verus` clean, `just verify-creusot`
unaffected (`Proved (112 files) ✔`).
...
### Step 8 — a real codegen layer replaces the hand-typed `verus_exchange!` call sites — done

Direct follow-on to `EXCHANGE_PROOF_DERIVATION_PLAN.md`'s own Step 9
(the Creusot codegen layer), not a self-initiated extension: "does
this lesson on codegen apply to verus or Kani exchange derives?"
Kani's own scaffold was already real proc-macro codegen from the
start (`#[amenable_derive::exchange(..)]`) — no translator-conflict
problem to fix there. Verus was the real, live gap: Step 7 generated
the `Witness<V>`/`Exchange<V>` *definitions* (`exchange_support::
verus_exchange!`, a `macro_rules!` macro — the only kind `verus
--crate-type=lib` can resolve at all, per this plan's own "Known
gotchas"), but every real edge's own *call site* (`verus_exchange!(
Stoplight, input: .., .., .., Yellow, GalleryVerifier, { <body> })`)
was still hand-typed and hand-copied into `gallery::
stoplight_exchange`, with nothing guarding drift between it and
Kani's own real body the way `amenable_creusot`'s former mirror at
least had `stoplight_mirror_consistency_test.rs` for. Exactly the gap
Step 9's own codegen closed on the Creusot side.

**What got built.** A new `amenable::verus_exchange_export` module
(mirroring `creusot_export.rs`'s own architecture, itself mirroring
`verus_export.rs`) plus `emit-verus-exchange-companions` CLI command
reads the same `amenable_core::ExchangeEdgeRecord` registry
`emit-creusot-companions` already reads — `#[amenable_derive::
exchange(..)]`'s real, verbatim-captured Kani transition bodies,
registered unconditionally since `amenable_kani` is ordinary
Cargo-built and never translated by anything — and writes one real
`crate::exchange_support::verus_exchange!(..)` call per edge into
`amenable_verus/src/gallery/generated/stoplight_exchange/*.rs`,
`include!`d (not `mod`-declared, so the generated files share
`stoplight_exchange.rs`'s own scope with no imports needed) from
`gallery::stoplight_exchange`. Structurally simpler than the Creusot
generator in one real respect: `verus_exchange!`'s own macro
definition already threads the real predicate through automatically
(`ensures <$evidence as crate::Ensures<$verifier>>::ensures(result)`,
resolved via whichever `verus_ensures!` call already registered for
that evidence type), so there is no trivial-contract placeholder to
hardcode in the generator at all — unlike Creusot's generator, which
still hardcodes `#[requires(true)] #[ensures(true)]` since no
`creusot_ensures!`-style mechanism exists yet. `verus_ensures!`
itself, and the surrounding state/token/sidecar type definitions,
stay hand-written — same scope discipline as Step 9.

**No Creusot-style bootstrap chicken-and-egg problem.**
`amenable_verus` is never a Cargo dependency of `amenable` (Verus
never resolves `Cargo.toml` at all — see this plan's own "Known
gotchas"), so `emit-verus-exchange-companions` needed no placeholder
commit to become runnable; the only real constraint was that
`amenable_verus`/`verus --crate-type=lib` itself needs the generated
files to exist on disk before *it* can compile, satisfied by simply
running the generator once before the first real check.

**Real fixes carried over from Step 9, confirmed to reproduce
identically rather than assumed:** the same `stringify!` one-space-
per-token join (`Established < Green , GreenToken >`), the same
`rustfmt`-doesn't-discover-`include!`d-files gap (fixed by invoking
`rustfmt` directly on the generated files in the new `generate-verus-
exchange` justfile recipe, not `cargo fmt -p amenable_verus`), the
same captured-body first-line dedent bug, and the same `//!` → `//`
fix (an inner doc comment is a real `E0753` once `include!`d
mid-file). `tidy_stringified_type`/`indent_body` are duplicated
between `creusot_export.rs` and `verus_exchange_export.rs` rather than
shared, matching Step 9's own reasoning (structurally different
output, different crates' own `AmenableResult` plumbing).

**One new fix, specific to this side:** capturing the real `Ok(..)`-
wrapped body (rather than the gallery's former hand-simplified one)
required `gallery::stoplight_exchange`'s own `StoplightError` mirror
type — already added ahead of this step, matching Step 9's identical
`amenable_creusot::stoplight::StoplightError` — to also derive
`Debug`, since `full_cycle`'s `.unwrap()` calls need it (`amenable_
creusot`'s own copy already had `Clone, Copy` for an unrelated reason;
`Debug` was the one still missing here).

**Verified for real, against the actual toolchain, not assumed**:
`verus --crate-type=lib`: `419 verified, 0 errors`. A real, non-
vacuous regression check — swapping the generated `Green -> Yellow`
edge's real `Ok(..)` body for `Err(StoplightError::NotUsed)` directly
in the generated file — produced the identical class of failure Step
7's own regression check found (`error: postcondition not satisfied`,
pointing at the exact macro-generated `ensures` clause and the exact
line that violates it), reverted by *regenerating* rather than a
manual `git checkout` (proving the generation loop itself, not just
its one-time output). Full workspace `check`/`clippy --all-targets -D
warnings`/`test` clean per-package, `just check-all-verus` clean, `just
verify-verus` clean end to end through the new `generate-verus-
exchange` recipe (wired into `check-verus`/`clippy-verus`/`test-verus`/
`verify-verus`, matching `emit-verus-witnesses`'s own "regenerate
before checking" placement and `generate-creusot`'s identical
wiring). Step 8 is complete.
...
Steps 0 through 8 are complete. One real next step remains, not
...
and the same reason Step 8's own generator has no predicate-content
placeholder to worry about (unlike Creusot's, per Step 9's own "Left
for explicit future direction"). A coarser-grained generalization also
remains, not started: `Evidence`/`ProofToken`/`Establish` themselves,
and the `Established<T, Token>`/`Sidecar<V>` carrier shape, are still
declared by hand once per state machine (not once per edge, unlike the
`Witness`/`Exchange`/`Ensures` wiring Steps 6/7/8 already cover); and
migrating `amenable_std`'s own much larger Verus witness-bridge surface
to this same call-site-codegen pattern, the direct Verus-side
counterpart of Step 9's own deferred question for Creusot.
